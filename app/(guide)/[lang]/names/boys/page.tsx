...
export async function generateMetadata({
  ...
  const path = (l: Locale) => `${SITE_URL}/${l}/names/boys`;
  const languages: Record<string, string> = {};
  for (const l of LOCALES) languages[HREFLANG[l]] = path(l);
  languages["x-default"] = path(X_DEFAULT_LOCALE);

  return {
    title: NAMES_BOYS.metaTitle[locale],
    description: NAMES_BOYS.description[locale],
...
export default async function BoyNamesPage({
  ...
  const boys = (await getAllNames()).filter((n) => n.gender === "male");
